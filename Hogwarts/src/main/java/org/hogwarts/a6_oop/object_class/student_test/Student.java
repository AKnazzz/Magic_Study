...
public class Student {
    ...
    public Student(int studentId, String name, String surname, int course, double mathAverageGrade,
            double economicsAverageGrade, double englishAverageGrade) {
...
class StudentTest {
    ...
    static double sredOcenkaForStudent(Student student) {
        double sredOcenka = (student.mathAverageGrade + student.economicsAverageGrade + student.englishAverageGrade)
                / 3;
        System.out.println(
                "Средняя арифметическая оценка студента " + student.name + " " + student.surname + " = " + sredOcenka);
        return sredOcenka;
    }
}
