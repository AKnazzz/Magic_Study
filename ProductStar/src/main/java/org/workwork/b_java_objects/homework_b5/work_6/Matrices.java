package org.workwork.b_java_objects.homework_b5.work_6;
...
public class Matrices {
    ...
    public static class Vector {
        ...
        public Vector(int n) {
            if(n <= 0){
                throw new IllegalArgumentException("Длина вектора должна быть больше нуляо");
            }
        ...
        public Vector add(Vector other) {
            if(this.items.length != other.items.length) {
                throw new IllegalArgumentException("Не равные размерности векторов");
            }
            int[] newItems = new int[this.items.length];
            for(int i = 0; i < this.items.length; i++) {
                newItems[i] = this.items[i] + other.items[i];
            }
        ...
        public int dotProduct(Vector other) {
            ...
            int newItems = 0;
            for(int i = 0; i < this.items.length; i++) {
                newItems += this.items[i] * other.items[i];
            }
            return newItems;
        }
        ...
        public Vector scalarMultiply(int scalar) {
            ...
            int[] newItems = new int[this.items.length];
            for(int i = 0; i < this.items.length; i++) {
                newItems[i] = this.items[i] * scalar;
            }
            return new Vector(newItems);
        }
    ...
    public static class Matrix {
        ...
        public Matrix add(Matrix other) {
            if(this.nRows != other.nRows || this.nCols != other.nCols) {
                throw new IllegalArgumentException("Не равная размерность матриц");
            }
            Matrix newItems = new Matrix(this.nRows, this.nCols);
            for(int i = 0; i < this.nRows; i++) {
                for(int j = 0; j < this.nCols; j++) {
                    newItems.rows[i][j] = this.rows[i][j] + other.rows[i][j];
                }
            }
            return newItems;
        }
        ...
        public Matrix scalarMultiply(int scalar) {
            ...
            for(int i = 0; i < this.nRows; i++){
                for(int j = 0; j < this.nCols; j++){
                    newItems.rows[i][j] = this.rows[i][j] * scalar;
                }
            }
            return newItems;
        }
        ...
        public Matrix transpose() {
            ...
            for(int i = 0; i < this.nRows; i++){
                for(int j = 0; j < this.nCols; j++){
                    newItems.rows[i][j] = this.rows[i][j];
                }
            }
            return newItems;
        }
...
// Базовый случай для матрицы 2x2
            ...
            for (int i = 0; i < matrix[0].length; i++) {
                ...
                for (int j = 1; j < matrix.length; j++) {
                    for (int k = 0, colIndex = 0; k < matrix[0].length; k++) {
                        if (k != i) {
                            smallerMatrix[j - 1][colIndex++] = matrix[j][k];
                        }
                    }
                }
